...
\chapter{Flatland}\label{ch:flatland}
...
\begin{equation}
	V_t^{(N)} = \sum_{i = t}^{t + N - 1} \gamma^{i - t}r_i + \gamma^{N}V(s_{t + N})\label{eq:N-step}
\end{equation}

where $V$ is a value function approximator and $N$ controls the bias–variance tradeoff of the estimator.
Bigger $N$ values results in an estimator closer to empirical returns and induce less bias and more variance.
On the other hand GAE is defined in the following way:

\begin{equation}
	V_t^{(GAE)} = (1 - \lambda)\sum_{N > 0} \lambda^{N - 1}V_t^{(N)}\label{eq:GAE}
\end{equation}
