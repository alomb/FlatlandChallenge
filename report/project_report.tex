...
References explore more alternatives and approaches involving the optimizer and its hyperparameters as learning rate decay and momentum but we did not cover them.
...
\begin{itemize}
	...
	In the Flatland Baselines this information is used to select the steps in which the action of an agent must be calculated by the neural network (and in our implementation, then this happens, the memory is also updated with the observation that produced the action).
