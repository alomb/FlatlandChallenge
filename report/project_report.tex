...
If a MOVE\_* action is performed during this state it becomes ACTIVE\@.
	\item ACTIVE (1) the agent is in the grid (position is not None) and has not reached the target yet.
...
This does not mean that the action will be successfully executed due to the presence of malfunctions or blocking agents, in this case info\_dict["action\_required"] will remain True.
...
This means that agents that are arrived to their destination and many agents that are in deadlock (by trying to move they increase their position fraction) can be ignored using this single flag.
	\item Is an agent which has reached DONE removed automatically in the following step?
Although the agent may still be rendered, the agent is removed from the grid.
...
Deadlocks occur when two or more agents are stucked in a particular situations which does not let anyone of them moving without colliding with another agent or moving in an invalid cell.
\end{itemize}
...
The \textbf{StatsWrapper} also computes the probability distribution of the actions taken during each episode.
...
Training data can be set and handled using the methods from the common class \textbf{Policy}.
	\item Time metrics, related to step and reset the environment and training and updating the neural networks.
...
This is not a new situation in DRL, as reported by~\citep{Hernandez-Leal-2019}, training involves dozens of CPUs and millions of frames usually requiring entire days of continuous computations.
We trained and tested our code mainly on Google Colaboratory, leveraging the possibility to run many concurrent instances and the high number of available threads in CPU runtime type, as we observed that the major bottlenecks are in the Flatland code.
...
Due to the greater scalability we have performed the experiments mostly using Local Trees as they depends on some parameters which are independent of the size of the grid and the number of agents.
As the name suggests the Local Tree observation has a tree-like structure.
The observation vector is composed of 4 sequential parts, corresponding to the data from the possible movements in a RailEnv, sorted relatively to the current orientation of the agent, rather than the classical NESW as used in the transitions.

In particular, each branch data is organized as:
...
Let's consider an environment with 3 agents, in this case the main behavior is the first because the probability of a collision is not very relevant, but if we consider the same environment with 10 agents the skill to avoid deadlocks is decisive for the overall performance.
...
Anyway we have implemented a wrapper class called \textbf{RewardsWrapper} to flexibly choose during the training phase whether to use the rewards shaped or the standard Flatland's reward system.
...
\begin{itemize}
	\item If $reward\_shaped$ is less than 0 then it is divided by the absolute value $deadlock\_penalty$ (In this way the worse reward obtainable is -1 considering deadlock as the worst case).
...
\end{figure}
Instead of inserting the agent's id, which might mislead the network to associate an id to a specific set of agent's features such as speed and malfunction rate which are only specific of a certain generated environment, we formulated a function to transform agent's features to ids.
During an execution, agents with the same speed have the same id, so are seen uniformly as we see them from the rendering engine, hopefully this strategy may leverage the network to recollect what it has learnt and provide a correct specific behaviour based on speed profiles and malfunction stochasticity.
...
Hidden states and cell states are stored in the memory to allow this operation.
	\item When the trajectory is used to update the policy sequences of transitions are directly passed to the network and except from the first hidden state and cell state, the others produced at each call are managed by the PyTorch library.
\end{itemize}
...
On the other hand, Flatland's action space is discrete and we decided to model it with a Categorical Distribution, a generalization of a Bernoulli distribution for a categorical random variable.
...
References explore more alternatives and approaches involving the optimizer and its hyperparameters as learning rate decay and momentum, but we did not cover all of them.
...
The~\ref{subsec:testing} describes the consequences on the performance caused by these changes.
\\
\\
...
We decided to follow the OpenAI Baseline's implementation, observing better performance, as it leads to higher diversity data in each mini-batch even without updating the advantages.
We left also the opportunity to disable any type of shuffling from the the hyperparameters initialization.
...
To calculate the advantage $A_t^{(N)} = V_t^{(N)} - V^{\pi}(s_t)$ are necessary the estimates of the action value $V_t^{(N)}$ and the state value $V^{\pi}(s_t)$.
As is easier and cheaper to get the Q value from V, Q is estimated using the equation~\ref{fig:N-step}.
\begin{figure}
	\[ V_t^{(N)} = \sum_{i = t}^{t + N - 1} \gamma^{i - t}r_i + \gamma^{N}V^{\pi}(s_{t + N}) \]
	\caption{The N-step formula used to obtain an estimate of the Q value from $V^{\pi}$. $V^{\pi}$ is a value function approximator, $N$ controls the bias–variance trade-off of the estimator and $\gamma$ is the discount factor.}
	\label{fig:N-step}
\end{figure}
While the summed actual rewards tend to have less bias and more variance, as they come from a single trajectory, $V^{\pi}$ tends to be more biased because it is computed with a function approximator, but has also lower variance since it reflects an expectation over all of the trajectories seen so far.
...
\\
\begin{figure}
	\[ A^{(GAE)} = (1 - \lambda)\sum_{N > 0} \lambda^{N - 1}A_t^{(N)} \]
	\caption{The GAE calculated with n-step forward returns. $\lambda$ is a hyperparameter between 0 and 1 which controls the bias–variance trade-off. Usually this formula is too complex to work it so an alternative definition is used.}
	\begin{gather*}
	    \delta_{t} = r_{t} + \lambda V^{\pi}(s_{t + 1}) - V^{\pi}(s_{t})\\
	    A^{(GAE)} = \sum_{N > 0} (\lambda \gamma)^{N} \delta_{t + N}\\
	\end{gather*}
	\caption{The alternative formula used in the code implementation.}
	\label{fig:GAE}
\end{figure}
...
So with this parameter configuration for Reward Shaping, our desire is to penalize more deadlocks than \textbf{$stop\_penalty$}, \textbf{$shortest\_path\_penalty\_coefficient$} and \textbf{$done\_bonus$}, which are mainly related to reaching the destination in the least number of steps.
The plots in figure~\ref{fig:psppo_training_rs}, clearly show that \textbf{Reward Shaping} has a positive impact on the performance.
Apparently the only metrics where disabling the \textbf{Reward Shaping} seems to perform better are those concerning deadlocks.
However we have noticed that this effect is due to the learning of a polarized policy on the STOP\_MOVING: when an agent is stationary most of the time it is less likely to go into a deadlock and this is also the reason for decreasing trend of the other metrics.

\begin{figure}
	\begin{subfigure}{\linewidth}
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_1/score}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_1/completion}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_1/deadlocks}
	\end{subfigure}\par\medskip
	\begin{subfigure}{\linewidth}
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_1/accumulated_score}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_1/accumulated_completion}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_1/accumulated_deadlocks}
	\end{subfigure}\par\medskip
	\caption{Training using reward shaping compared to using only standard rewards.}
	\label{fig:psppo_training_rs}
\end{figure}
...
\end{lstlisting}
Another variant tested arises from a different interpretation of the trajectory of each agent, which considers the tuples $(s, a, r, s')$ as part of a single episode until the agent does not reach its destination.
This highly influences the advantage estimation and was mentioned in~\ref{subsubsec:training-setup2}.
...
\begin{itemize}
	...
	In the Flatland Baselines this information is used to select the steps in which the action of an agent must be calculated by the neural network (and in our implementation, follows that the memory is updated with the observation that produced the action).
	...
	\begin{enumerate}
		...
		\item [2.] When an agent goes into deadlock the memory is updated apriori, this is very important in the case of agents with speeds less than 1.
According to the previous logic, if the deadlock occurred while an agent was walking through a fraction of the cell, "action\_required" was set to False and the memory was not updated with the observation of the deadlock (this is particularly limiting when using Reward Shaping).
	\end{enumerate}
\end{itemize}

It is possible to observe in the figure~\ref{fig:psppo_training_rec_loop} that the modification of the loop combined with the use of \textbf{LSTM} brings a noticeable improvement.
\begin{figure}
	\begin{subfigure}{\linewidth}
	...
	\end{subfigure}\par\medskip
	\caption{The different combinations obtained using recurrence and the modification on the loop during the training.}
...
We have performed other experiments in order to investigate the best learning rate for the final training and also to show the importance of \textbf{Action Masking} described in ~\ref{subsec:actions}, which has been activated the tests described above without explicitly providing any evidence of its positive impact in the performance.
...
Even the hyperparameters for training are the same as before, varying only the learning rate and the \textbf{Action Masking} flag as the names suggest.
\begin{figure}
	\begin{subfigure}{\linewidth}
	...
	\end{subfigure}\par\medskip
	\caption{The comparison between action masking or not, with different learning rates.}
...
For example assigning $16$ would mean that the 16\textsuperscript{th} new insertion in the memory triggers the network to learn.
		\item "fingerprints": enable fingerprints.
		\item "fingerprint\_type": fingerprint data.
If fingerprints is enabled and this is not set the default value is FingerprintType.EPSILON\_STEP\@.
Other options are FingerprintType.EPSILON\_EPISODE, FingerprintType.EPISODE etc.. (see FingerprintType class).
	\end{itemize}
	\item Memory
	\begin{itemize}
...
In particular, we observed different positive trends:
\begin{itemize}
...
\href{https://app.wandb.ai/lomb/flatland-challenge-ps-ppo-curriculum}{Project page}.\\

The first test conducted on an older version of the PS-PPO algorithm was divided in three phases: the first was the application of the semi-automatic curriculum using the five different environments, obtained with the seeds 19, 80, 130, 200, the second consisted in training the environments for more than two thousands episodes and finally the latter concerned the evaluation of them.
These experiments demonstrated that the curriculum is able to preserve or slightly improve the performances.
Finally we applied the mentioned three phases of curriculum learning to the final PS-PPO model (seed 14) obtaining better metrics during the training, as visible in the plots in graph~\ref{fig:training_cl}, but experiencing worse results on the evaluation phase as shown in the tables~\ref{tab:evaluation_cl1} and~\ref{tab:evaluation_cl2}.

\begin{figure}
	\begin{subfigure}{\linewidth}
		\includegraphics[width=0.33\linewidth]{resources/charts_cl/score}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_cl/completion}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_cl/deadlocks}
	\end{subfigure}\par\medskip
	\begin{subfigure}{\linewidth}
		\includegraphics[width=0.33\linewidth]{resources/charts_cl/accumulated_score}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_cl/accumulated_completion}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_cl/accumulated_deadlocks}
	\end{subfigure}\par\medskip
	\caption{psppo\_final\_cl is the training based on curriculum learning while the other is the best PS-PPO model both trained on small environments with malfunctions and speeds.}
	\label{fig:training_cl}
\end{figure}
...
At the end, we performed the final training phase using the best combination of hyperparameters and algorithm that came to light from the experiments in~\ref{subsec:testing} to generate two models:
\begin{itemize}
...
Although the work has presented multiple and serious studies and modifications to the policy and to the network, the most remarkable improvements occurred changing the training loop, for instance changing the agents conditions to update the network, flag the termination of the episodes or aggregate the rewards.
As presented in many MADRL works involving partial observability, recurrent models can provide better results and help the model get more insights on the observations.
...
Unfortunately our naive curriculum learning did not bring better results, suggesting more studies and better approaches should be performed.
...
It is possible to prune parts of the computation graph setting Tensor's attribute \textit{requires\_grad} to False, or detaching tensors from the current graph with the \textit{detach()} method.
Alternatively, it is possible to enclose code portions within a \textit{with torch.no\_grad()} block to temporarily prevent gradients tracking.
...
\begin{table}[]
	...
	\multicolumn{1}{|c|}{}                         & \multicolumn{3}{c|}{Medium size} & \multicolumn{4}{c|}{Big size}                          \\ \cline{2-8}
	\multicolumn{1}{|c|}{}                         & 3 agents  & 5 agents  & 7 agents & 5 agents  & 7 agents  & \multicolumn{2}{l|}{10 agents} \\ \hline
	accumulated\_completion                        & 0.8963    & 0.8006    & 0.7004   & 0.8276    & 0.7283    & \multicolumn{2}{l|}{0.3912}    \\ \hline
	accumulated\_deadlocks                         & 0.0950    & 0.1912    & 0.2869   & 0.1602    & 0.2573    & \multicolumn{2}{l|}{0.5403}    \\ \hline
	accumulated\_score                             & -0.186    & -0.275    & -0.364   & -0.264    & -0.351    & \multicolumn{2}{l|}{-0.333}    \\ \hline
	\end{tabular}
	\caption{Curriculum learning performances on simpler environments.}
	\label{tab:evaluation_cl1}
	...
	\multicolumn{1}{|c|}{}                         & \multicolumn{3}{c|}{Medium size} & \multicolumn{4}{c|}{Big size}                          \\ \cline{2-8}
	\multicolumn{1}{|c|}{}                         & 3 agents  & 5 agents  & 7 agents & 5 agents  & 7 agents  & \multicolumn{2}{l|}{10 agents} \\ \hline
	accumulated\_completion                        & 0.7483    & 0.6316    & 0.5334   & 0.5900    & 0.4979    & \multicolumn{2}{l|}{0.6243}    \\ \hline
	accumulated\_deadlocks                         & 0.1540    & 0.2886    & 0.4079   & 0.2852    & 0.4019    & \multicolumn{2}{l|}{0.3582}    \\ \hline
	accumulated\_score                             & -0.192    & -0.239    & -0.280   & -0.251    & -0.292    & \multicolumn{2}{l|}{-0.440}    \\ \hline
	\end{tabular}
	\caption{Curriculum learning performances on more complex environments.}
	\label{tab:evaluation_cl2}
\end{table}

\begin{figure}
	\begin{subfigure}{\linewidth}
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_4/score}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_4/completion}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_4/deadlocks}
	\end{subfigure}\par\medskip
	\begin{subfigure}{\linewidth}
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_4/accumulated_score}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_4/accumulated_completion}\hfill
		\includegraphics[width=0.33\linewidth]{resources/charts_psppo_4/accumulated_deadlocks}
	\end{subfigure}\par\medskip
	\caption{The final training of PS-PPO. \href{https://wandb.ai/fiorenzoparascandolo/flatland-challenge-final}{Project page}}
	\label{fig:psppo_training_final}
\end{figure}
