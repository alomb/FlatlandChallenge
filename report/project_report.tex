...
%TODO: tensorboard, more words on wrappers in general and deadlock detections
...
During an execution, agents with the same speed have the same id, so are seen uniformly as we see them, hopefully this strategy may leverage the network to recollect what it has learnt and provide a correct specific behaviour.
...
% TODO: picture difference one to the other
...
%TODO: evaluation mode, horizon and batch, reward shaping, masking, skipping, allow no op
...
Our D3QN uses also different experience replay strategies as described in~\ref{subsubsec:memory}.
...
The following part will continue describing a soft adaptation to multi-agent scenario which relies mostly on the classical implementation while~\ref{subsubsec:fingerprints} describes more specific tricks. %TODO expand soft adaptation
...
\subsubsection{Experience Replay}\label{subsubsec:memory}
...
\begin{itemize}
	\item Network Architecture
	\begin{itemize}
		\item "double\_dqn": True
		\item "shared": False
		\item "hidden\_size": 256
		\item "hidden\_layers": 2
		\item "learning\_rate": 0.52e-4
		\item "gamma": 0.99
		\item "tau": 1e-3
	\end{itemize}
	\item Training setup
	\begin{itemize}
		\item "n\_episodes": 2500
        \item "eps\_decay": 0.99
        \item "eps\_start": 1.0
        \item "eps\_end": 0.01
		\item "batch\_size": 32
		\item "buffer\_min\_size": 0
		\item "update\_every": 8
	\end{itemize}
	\item Memory
	\begin{itemize}
		\item "buffer\_size": int(1e6)
		\item "memory\_type": "per" ("uer" or "per")
	\end{itemize}
	\item Action Masking and Skipping
	\begin{itemize}
		\item "action\_masking": False
...
% TODO hierarchical https://openai.com/blog/learning-a-hierarchy/, meta, conv
