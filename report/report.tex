...
\begin{algorithm}[H]
	...
	\eIf{agent.moving}{
		update agent.speed\_data['position\_fraction'] then if it is completely arrived on the next cell, it updates position, direction and clears completion percentage and finally performs the action stored in agent.speed\_data['transition\_action\_on\_cellexit'] as soon as the next cell is free\\
		reward is computed
	}{
		reward is computed
	}
