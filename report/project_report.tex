...
The first test conducted on an older version of the PS-PPO algorithm was divided in three phase: the first was the application of the semi-automatic curriculum using the five different environments, obtained with the seeds 19, 80, 130, 200, the second consisted in training the environments for more than two thousands episodes and finally the latter the evaluation of them.
These experiments demonstrate that the curriculum is able to preserve or slightly improve the performances.
Finally we applied the mentioned three phases of curriculum learning to the final PS-PPO model obtaining better metrics during the training as visible in the plots in graph~\ref{fig:training_cl}, but experiencing worse results on the evaluation phase as shown in the tables~\ref{tab:evaluation_cl1} and~\ref{tab:evaluation_cl2}.
